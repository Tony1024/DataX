    ...
    public static class Task {
        ...
        public void init(Configuration readerSliceConfig) {
            ...
            try {
                ...
                this.sourceClass = Class.forName(readerSliceConfig.getString("com.sinohealth.datax.entity.source." + com.alibaba.datax.plugin.rdbms.writer.Key.SOURCE_CLASS));
                this.targetClass = Class.forName(readerSliceConfig.getString("com.sinohealth.datax.entity.target." + com.alibaba.datax.plugin.rdbms.writer.Key.TARGET_CLASS));
            ...
            // 加载检验标准化字典
            Map<String, BasTestItem> testItemMap = bizDataIniter.findBasTestItemMap();
            // 加载检验单位标准化字典
            ...
            // 加载诊断标准化字典
            Map<String, BasDiagnoseItem> diagnoseItemMap = bizDataIniter.findBasDiagnoseItemList();
            //加载检查结果初始化标准字典
            List<BasCheckItemInit> list = bizDataIniter.findCheckItemInit();
            // 加载离散类型为2的字典
            ...
            this.commonData.setBasDiagnoseItemMap(diagnoseItemMap);
            this.commonData.setBasCheckItemInitList(list);
                ...
                if (CollectionUtil.isNotEmpty(sourceList)) {
                    for (Object sourceEntity : sourceList) {
                        if (sourceEntity != null) {
                            ...
                            if (targetEntity instanceof DiagnoseResultDetailTList) {
                                List<DiagnoseResultDetailT> details = ((DiagnoseResultDetailTList) targetEntity).getDiagnoseResultDetailList();
                                if (CollectionUtil.isNotEmpty(details)) {
                                    for (DiagnoseResultDetailT detail : details) {
                                        // create record
